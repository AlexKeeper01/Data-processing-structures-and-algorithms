...
void printTableWithCodes(vector<Symbol>& symbols, const string& input) {
    ...
    sort(symbols.begin(), symbols.end(), [&](const Symbol& a, const Symbol& b) {
        ...
        for (auto& symbol : symbols) {
            ...
            cout << setw(10) << symbol.frequency
                << setw(15) << symbol.probability
                << setw(24) << symbol.code << endl;
        }
}

void calculateCompressionRationASCIIUniform(const string& input, vector<Symbol>& symbols) {
...
int main() {
    ...
    string input = parseFile("input.txt");
    ...
    calculateCompressionRation(input, encodedText);
